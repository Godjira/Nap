...
func pick_direction_move(delta):
	if is_dead : return
...
func _on_mob_sensor_body_entered(body):
...
func _on_mob_sensor_body_exited(body):
...
func _on_mob_sensor_area_entered(area):
	...
	for direction in all_directions:
		...
		if not "get_collision_mask" in a:
			return

		for index in [1, 2, 4]:
			var have_col = a.get_collision_mask_value(index)
			var have_layer = a.get_collision_layer_value(index)
