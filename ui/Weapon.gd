extends Panel
class_name InventorySlotWeaponUI
...
func attach_player(p:Character0):
	...
	if player.active_weapon:
		item_visual.texture = player.active_weapon.texture
		inventory_slot = InventorySlot.new()
		inventory_slot.is_armor_slot = false
		inventory_slot.is_weapon_slot = true
		inventory_slot.amount = 1
		inventory_slot.item = player.active_weapon
		self.update(inventory_slot)
...
func _on_mouse_entered():
	...
	if inventory_ui and inventory_slot.item:
		...
		inventory_ui.tooltip.set_item_description("damage: " + str(inventory_slot.item.damage))
...
func _can_drop_data(at_position:Vector2, data) -> bool:
	return data is Panel and data.inventory_slot and data.inventory_slot.item is WeaponItem

func _drop_data(at_position:Vector2, data) -> void:
	var t = self.inventory_slot
	print(data, t)
	if data.inventory_slot.item:
		player.active_weapon = data.inventory_slot.item
